...
await tocar('#int-gerar');await esperarRecomendacao();await tocar('#int-lote [data-rec-dezenas]');
...
checar('base real informa lacunas e contagens por interseção independente',await js(`const r=S.recResultados['mega-sena'],alvo=new Set([2,4,35,46,54,60]),ids=S.resultados.map(x=>Number(x.concurso));return r.base.hist.length===new Set(ids).size&&r.base.lacunas===Math.max(...ids)-Math.min(...ids)+1-new Set(ids).size&&r.faixas.every(f=>f.exatos.vezes===S.resultados.filter(x=>x.dezenas.filter(d=>alvo.has(d)).length===f.acertos).length)`));
...
const ultimo418=Math.max(...history418.map(r=>Number(r.concurso))),semData418=history418.filter(r=>!r.data).length,exatosTres418=history418.filter(r=>r.dezenas.filter(d=>[14,23,53,56,57,60].includes(d)).length===3).length;
await cmd('Emulation.setTouchEmulationEnabled',{enabled:true,maxTouchPoints:1});
await cmd('Emulation.setDeviceMetricsOverride',{width:412,height:915,deviceScaleFactor:2,mobile:true});
...
checar('worker analisa todo o acervo real e 2.000 referências',await js(`const t=document.querySelector('#ll-content').textContent;return t.includes(${JSON.stringify(history418.length.toLocaleString('pt-BR')+' concursos disponíveis')})&&t.includes('2.000 jogos aleatórios')&&t.includes('Distribuição completa')`));
...
checar('histórico por faixa permite consultar coincidências e intervalos',await js(`const t=document.querySelector('#ll-concursos').textContent;return t.includes(${JSON.stringify(exatosTres418+' concursos')})&&t.includes('Coincidentes')&&t.includes('Não coincidentes')&&t.includes('Intervalo desde')`));
...
await tab418('gerador');await js(`document.querySelector('#ll-content > details').open=true;return true;`);await input418('#ll-quantidade',3);await input418('#ll-candidatos',200);await input418('#ll-orcamento',18);await input418('#ll-maxSobreposicao',2);await tocar('[data-ll-action="gerar"]');await wait418();
...
await input418('#ll-alvo',ultimo418+1);await tocar('[data-ll-action="salvar"]');
checar('lote prospectivo salvo e integrado aos jogos existentes',await js(`const m=Guardar.ler('laboratorio418',{});return m.lotes.length===1&&m.lotes[0].concursoAlvo===${ultimo418+1}&&m.lotes[0].geradoAte===${ultimo418}&&S.jogos.filter(j=>j.inteligencia?.versao==='4.18').length===3`));
...
checar('simulações não viram resultados reais',await js(`return document.querySelector('#ll-content').textContent.includes('2.000 simulações')&&document.querySelector('#ll-content').textContent.includes('nunca gravados')&&S.resultados.length===${history418.length}`));
await tab418('dados');checar('origem, ausências e importação ficam visíveis',await js(`return document.querySelector('#ll-content').textContent.includes(${JSON.stringify(semData418.toLocaleString('pt-BR')+' sem data')})&&!!document.querySelector('#ll-arquivo')`));
...
async function esperarRecomendacao(){
  ...
  while(Date.now()-start<45000){
    if(await js(`return !document.querySelector('#int-gerar')?.disabled`))return;
    await dormir(100);
  }
  throw Error('Recomendação excedeu o prazo');
}
secao('R. Histórico conectado à recomendação');
const rows419=JSON.parse(readFileSync(join(RAIZ,'dados/mega-sena.json'),'utf8')).concursos.slice(-120);
