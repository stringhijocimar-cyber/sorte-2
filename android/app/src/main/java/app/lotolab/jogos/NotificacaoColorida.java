...
final class NotificacaoColorida {
    ...
    static boolean valida(JSONObject cf) {
        ...
        return r != null && ConferenciaRegras.validos(numeros(cf.optJSONArray("dezenas")),r[3],r[4],r[0],r[1])
            && ConferenciaRegras.validos(numeros(cf.optJSONArray("sorteadas")),r[2],r[2],r[0],r[1]);
    }
    static Bitmap imagem(int[] jogo, int[] sorteadas) {
        int colunas=10, linhas=(jogo.length+colunas-1)/colunas;
        Bitmap bitmap=Bitmap.createBitmap(1000, 90+linhas*90, Bitmap.Config.ARGB_8888);
        ...
        p.setTextSize(26);p.setColor(Color.WHITE);
        canvas.drawText("ACERTOS EM VERDE  ·  NÃO SORTEADAS EM VERMELHO",30,38,p);
        for(int i=0;i<jogo.length;i++) {
            boolean acertou=false;for(int d:sorteadas)if(d==jogo[i]){acertou=true;break;}
            float x=50+(i%colunas)*100,y=90+(i/colunas)*90;
            p.setColor(acertou?Color.rgb(21,125,71):Color.rgb(252,230,232));canvas.drawCircle(x,y,34,p);
            p.setColor(acertou?Color.WHITE:Color.rgb(146,45,60));p.setTextSize(30);p.setTextAlign(Paint.Align.CENTER);
            canvas.drawText(String.format(Locale.ROOT,"%02d",jogo[i]),x,y-(p.ascent()+p.descent())/2,p);
        }
        return bitmap;
    }
    static synchronized boolean mostrar(Context c, JSONObject cf, String nome, boolean demonstracao) {
        ...
        if(!demonstracao&&prefs.getInt("enviado:"+modalidade,0)>=cf.optInt("concurso"))return true;
        ...
        String titulo=(demonstracao?"Demonstração · ":"")+nome+": "+acertos+" acertos";
        ...
        for(int d:jogo){boolean hit=false;for(int n:sorteio)if(n==d){hit=true;break;}
            descricao.append(String.format(Locale.ROOT,"%02d",d)).append(hit?" acertou; ":" não sorteada; ");}
        NotificationCompat.BigPictureStyle estilo=new NotificationCompat.BigPictureStyle()
            .bigPicture(imagem(jogo,sorteio)).setBigContentTitle(titulo).setSummaryText(texto)
        ...
        if(!demonstracao)prefs.edit().putInt("enviado:"+modalidade,cf.optInt("concurso")).apply();
        return true;
    }
}
